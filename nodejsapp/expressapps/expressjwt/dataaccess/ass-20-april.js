...
class DataAccess {
  ...
  async authUser(req, resp) {
    ...
    const userPermissions = user_permissions.filter((x) => user.username === x.username)[0].permissions
    // 3. Authenticate the user and issue token
    // do not pass the password
    const token = jsonwebtoken.sign({ user, userPermissions }, jwtSettings.jwtSecret, {
      expiresIn: 3600, // 1 hr
      algorithm: "HS384",
    });
